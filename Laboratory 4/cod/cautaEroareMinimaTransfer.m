function [indice, bloc_stanga, bloc_sus] = cautaEroareMinimaTransfer(bloc_stanga, bloc_sus, blocuri, pixeli, nrBlocuri, eroareTolerata, transfer)
    ...
    imagine_transfer = transfer.bloc_imagine_transfer;
    iteratieCurenta = transfer.iteratieCurenta;
    ...
    elseif stanga ~= 0
        ...
        for i = 1:nrBlocuri
            ...
            erori(i) = alpha * sum(sqrt(sum((stanga - dreapta).^2))) + (1-alpha) * (sum(sum(imagine_transfer - bloc_curent)).^2);
        end
    ...
    elseif sus ~= 0
        ...
        for i = 1:nrBlocuri
            ...
            erori(i) = alpha * sum(sqrt(sum((sus - jos).^2))) + (1-alpha)*(sum(sum(imagine_transfer - bloc_curent)).^2); 
        end
